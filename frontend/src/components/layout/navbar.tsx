...
import { MenuIcon, Users, MessageSquare, Briefcase, Calendar } from "lucide-react";
import {
  Sheet,
  SheetContent,
  SheetTrigger,
    ...
    <header className="fixed inset-x-0 top-0 z-50 backdrop-blur-sm border-b">
      ...
      >
        <div className="flex lg:flex-1">
          <Link to="/" className="-m-1.5 p-1.5 text-xl font-semibold">
            AlumniConnect
          </Link>
        </div>
        ...
        <div className="hidden lg:flex lg:gap-x-8">
          {navigation.map((item) => (
            <Link
              key={item.name}
              to={item.href}
              className="text-sm font-semibold leading-6 text-foreground hover:text-foreground/80 transition-colors flex items-center gap-2"
            >
